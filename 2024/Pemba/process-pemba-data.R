####################################################################.
#
#  Data Prep for Batchelor's thesis on Pemba social capital
#
####################################################################.
...
setwd("/Users/danielredhead/Dropbox/Pemba-wealth")

# Set write path
write_path <- "/Users/danielredhead/BAW-main/2024/Pemba"
...
write.csv(sur_m, paste0(write_path,"/data/relatedness_matrix.csv"))
...
sharing_unit <- select(su, household_ID = su_id, number_wives = nwives2018, wealth = su_direct_wealth)

sharing_unit$log_wealth <- log(sharing_unit$wealth)
...
# Create metrics for household table
average_r = as.data.frame(rowMeans(sur_m, na.rm = T))
outdegree_men_friends = as.data.frame(rowSums(male_k))
outdegree_men_work = as.data.frame(rowSums(male_q))
outdegree_women_friends = as.data.frame(rowSums(female_k))
outdegree_women_work = as.data.frame(rowSums(female_q))
indegree_men_friends = as.data.frame(colSums(male_k))
indegree_men_work = as.data.frame(colSums(male_q))
indegree_women_friends = as.data.frame(colSums(female_k))
indegree_women_work = as.data.frame(colSums(female_q))

colnames(average_r) <- "r"
colnames(outdegree_men_friends) <- "d"
colnames(outdegree_men_work) <- "d"
colnames(outdegree_women_friends) <- "d"
colnames(outdegree_women_work) <- "d"
colnames(indegree_men_friends) <- "d"
colnames(indegree_men_work) <- "d"
colnames(indegree_women_friends) <- "d"
colnames(indegree_women_work) <- "d"

sharing_unit$average_relatedness <- average_r$r[match(sharing_unit$household_ID,rownames(average_r))]
sharing_unit$outdegree_men_friends <- outdegree_men_friends$d[match(sharing_unit$household_ID,rownames(outdegree_men_friends))]
sharing_unit$outdegree_men_work <- outdegree_men_work$d[match(sharing_unit$household_ID,rownames(outdegree_men_work))]
sharing_unit$outdegree_women_friends <-outdegree_women_friends$d[match(sharing_unit$household_ID,rownames(outdegree_women_friends))]
sharing_unit$outdegree_women_work <- outdegree_women_work$d[match(sharing_unit$household_ID,rownames(outdegree_women_work))]
sharing_unit$indegree_men_friends <- indegree_men_friends$d[match(sharing_unit$household_ID,rownames(indegree_men_friends))]
sharing_unit$indegree_men_work <- indegree_men_work$d[match(sharing_unit$household_ID,rownames(indegree_men_work))]
sharing_unit$indegree_women_friends <-indegree_women_friends$d[match(sharing_unit$household_ID,rownames(indegree_women_friends))]
sharing_unit$indegree_women_work <- indegree_women_work$d[match(sharing_unit$household_ID,rownames(indegree_women_work))]

# Process sharing
union_sharing <- ifelse(share_mat1 == 1 | t(share_mat2) == 1, 1,0)
outdegree_union_sharing = as.data.frame(rowSums(union_sharing))
indegree_union_sharing = as.data.frame(colSums(union_sharing))
colnames(outdegree_union_sharing) <- "d"
colnames(indegree_union_sharing) <- "d"
sharing_unit$outdegree_union_sharing <- outdegree_union_sharing$d[match(sharing_unit$household_ID,rownames(outdegree_union_sharing))]
sharing_unit$indegree_union_sharing <- indegree_union_sharing$d[match(sharing_unit$household_ID,rownames(indegree_union_sharing))]

intersection_sharing <- ifelse(share_mat1 == 1 & t(share_mat2) == 1, 1,0)
outdegree_intersection_sharing = as.data.frame(rowSums(intersection_sharing))
indegree_intersection_sharing = as.data.frame(colSums(intersection_sharing))
colnames(outdegree_intersection_sharing) <- "d"
colnames(indegree_intersection_sharing) <- "d"
sharing_unit$outdegree_intersection_sharing <- outdegree_intersection_sharing$d[match(sharing_unit$household_ID,rownames(outdegree_intersection_sharing))]
sharing_unit$indegree_intersection_sharing <- indegree_intersection_sharing$d[match(sharing_unit$household_ID,rownames(indegree_intersection_sharing))]

# Create sum of years of eduction for each household
years_education <- as.vector(aggregate(years_education ~ su_id_1, att2, sum))
sharing_unit$years_education <- years_education$years_education[match(sharing_unit$household_ID, years_education$su_id_1)]

# Create average age for each household
average_age <- as.vector(aggregate(age ~ su_id_1, att2, mean))

sharing_unit$average_age <- average_age$age[match(sharing_unit$household_ID, average_age$su_id_1)]
# Create number of external ties
# Remove externals and subset to the two household sharing questions
net <- networks[!networks$alterid %in% att2$personid ,]
net$su_i <- att$su_id_1[match(net$personid, att$personid)]
# Add polygynous men to all of their associated SU's
...
net <- net[!duplicated(net$alterid, net$su_i),]
net$n_ties <- 1
external_ties <- as.vector(aggregate(n_ties ~ su_i, net, sum))
sharing_unit$sum_external_ties <- external_ties$n_ties[match(sharing_unit$household_ID, external_ties$su_i)]

write.csv(union_sharing, paste0(write_path, "/data/household_sharing_union.csv"))
write.csv(intersection_sharing, paste0(write_path, "/data/household_sharing_intersection.csv"))
write.csv(female_k, paste0(write_path, "/data/women_friendship.csv"))
write.csv(female_q, paste0(write_path, "/data/women_work.csv"))
write.csv(male_k, paste0(write_path, "/data/men_friendship.csv"))
write.csv(male_q, paste0(write_path, "/data/men_work.csv"))
write.csv(sharing_unit, paste0(write_path, "/data/household_attributes.csv"),row.names = FALSE)
